...
//    fileprivate func preloadArray(_stride: (Int, Int, Int), completionHandler: @escaping (_ array: [SKTexture]) -> ()) {
//
//            var array = [SKTexture]()
//
...
//    }
    ...
    fileprivate func turnPlane(direction: TurnDirection) {
        ...
        let forwardAnimation = SKAction.animate(with: array, timePerFrame: 0.05, resize: true, restore: false)
        let backwardAnimation = SKAction.animate(with: array.reversed(), timePerFrame: 0.05, resize: true, restore: false)
        
        let sequenceAction = SKAction.sequence([forwardAnimation, backwardAnimation])
